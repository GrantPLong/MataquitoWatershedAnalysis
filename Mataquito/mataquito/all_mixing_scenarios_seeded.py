...
sys.path.insert(0, str(Path(__file__).resolve().parent))  # so `import detritalFuncs_local` etc. work regardless of cwd

import detritalpy
from detritalpy import detritalFuncs as dFunc
import detritalFuncs_local
import detritalMixer_local as dMix
import detritalpy.adaptiveKDE as akde
import adaptiveKDE_local

# --- Patch in local, corrected KDE functions -- same pattern as the notebook ---
dFunc.KDE_bw_selector = detritalFuncs_local.KDE_bw_selector
dFunc.KDEcalcAges = detritalFuncs_local.KDEcalcAges
dFunc.KDEcalcAgesLocalAdapt = detritalFuncs_local.KDEcalcAgesLocalAdapt
dFunc.KDEcalcAgesGlobalAdapt = detritalFuncs_local.KDEcalcAgesGlobalAdapt
akde.ssvkernel = adaptiveKDE_local.ssvkernel
akde.sskernel = adaptiveKDE_local.sskernel
# --- end patch ---

# ─────────────────────────────────────────────────────────────────────────────
# Patch: dMix.perturb_by_normal_distribution can push a young grain's
# perturbed age slightly negative (error comparable to its own age). With a
# fixed numeric bw (e.g. bw=2), KDEcalcAges routes through KDEpy's FFTKDE,
# which requires every data point strictly inside the evaluation grid and
# raises ValueError("Every data point must be inside of the grid.") instead
# of tolerating it. Ages can't be negative anyway, so clipping here is both
# the fix and the physically correct behavior -- not just a workaround.
# This rebinds the name in detritalMixer's own module namespace, so every
# internal caller (resample_and_perturb_datesErrors, etc.) picks it up.
# ─────────────────────────────────────────────────────────────────────────────
_original_perturb_by_normal_distribution = dMix.perturb_by_normal_distribution

def _clipped_perturb_by_normal_distribution(means, std_dev):
    means_pert = _original_perturb_by_normal_distribution(means, std_dev)
    return np.clip(means_pert, X1 + XDIF, X2 - XDIF)

dMix.perturb_by_normal_distribution = _clipped_perturb_by_normal_distribution
...
BW, BW_X = 2, None                 # KDE bandwidth for mixing calc; only matters for 'r2-kde'
SIGMA = '2sigma'                   # input data uncertainty level

BASE_SEED_DEFAULT = 0              # overridable with --seed; combined with scenario+metric
                                    # to give each run its own deterministic, reproducible seed

N_BOOTSTRAP_ITERATIONS_DEFAULT = 10000  # overridable with --iterations
DO_PERTURB_RESAMPLED_AGES = True
N_GRAINS_TO_RESAMPLE = None

MAKE_HISTOGRAM_FIG = False  # coefficient_histograms figure -- set True to turn back on

# Every scenario now gets BOTH a zoomed (0-25 Ma) and full-range (0-400 Ma)
# version of any figure with age on the x-axis (fig1, fig3), rather than
# picking one range based on sample identity.
AGE_AXIS_RANGES = [(0, 25, '0-25Ma', 5), (0, 400, '0-400Ma', 8)]  # (xaxis_1, xaxis_2, tag, w1 for fig1)


def seed_for(scenario_name, objective_metric, base_seed):
    """
    Deterministic seed for one (scenario, metric) run, derived from a stable
    hash -- NOT Python's built-in hash(), which is randomized per-process by
    default and would give a different seed every time the script is run.

    Tying the seed to the scenario name + metric (rather than to a running
    counter) means rerunning a single scenario in isolation reproduces
    exactly the same bootstrap draws it got inside a full batch run, and
    trimming/reordering ACTIVE_SCENARIOS never changes any other scenario's
    result.
    """
    key = f"{base_seed}_{scenario_name}_{objective_metric}"
...
# ─────────────────────────────────────────────────────────────────────────────
def confirm_run_plan(scenarios, active_names, metrics, n_iterations, base_seed):
    ...
    print(f"Base seed: {base_seed} (each scenario+metric gets its own derived, reproducible seed)")
    print(f"Age-axis figures (fig1, fig3): both 0-25 Ma and 0-400 Ma versions generated per scenario\n")
    for s in active:
        parents_str = ','.join(s['parent_list'])
        children_str = ','.join(s['child_list'])
        print(f"  {s['name']:<28} parents=[{parents_str}]  children=[{children_str}]")
...
# ─────────────────────────────────────────────────────────────────────────────
def run_scenario_metric(scenario, objective_metric, main_byid_df, n_bootstrap_iterations, base_seed):
    name = scenario['name']
    parent_list = scenario['parent_list']
    child_list = scenario['child_list']
    tag = f"[{name}] {objective_metric}"

    # Seed the global numpy RNG deterministically for this scenario+metric.
    # Every randomness call in detritalMixer.py (resample_datesErrors,
    # perturb_by_normal_distribution) draws from this same global state via
    # np.random.*, so seeding it here makes the entire run -- best-fit search,
    # bootstrap, self-comparison -- exactly reproducible on rerun.
    seed = seed_for(name, objective_metric, base_seed)
    np.random.seed(seed)
    ...
    write_results_block(results_file, "Run parameters", [f"seed={seed}", f"n_bootstrap_iterations={n_bootstrap_iterations}"])
    ...
    # --- fig1: age distribution + mixture (plotMix) -- both age-axis ranges ---
    for xaxis_1, xaxis_2, range_tag, w1 in AGE_AXIS_RANGES:
        fig1 = dMix.plotMix(
            ...
            xaxis_1=xaxis_1, xaxis_2=xaxis_2, w1=w1, w2=4, c=4, plotPie=True, plotMixResults=True,
        ...
        fig1.savefig(os.path.join(MIXING_FIG_DIR, f'dPy-mix_{parent_str}_to_{child_str}_age_distribution_{metric_tag}_{range_tag}.pdf'))
        plt.close(fig1)
    ...
    # --- fig3: modelled vs. observed distribution comparison -- both age-axis ranges ---
    for xaxis_1, xaxis_2, range_tag, _w1_unused in AGE_AXIS_RANGES:
        ...
        fig3.savefig(os.path.join(MIXING_FIG_DIR, f'dPy-mix_{parent_str}_to_{child_str}_distribution_comparisons_{metric_tag}_{range_tag}.pdf'))
        plt.close(fig3)
...
def main():
    ...
    parser.add_argument('--metric', choices=['vmax', 'r2-kde', 'both'], default='both',
                         help="Which objective metric(s) to run (default: both).")
    parser.add_argument('--seed', type=int, default=BASE_SEED_DEFAULT,
                         help=f"Base seed for reproducible bootstrap draws (default: {BASE_SEED_DEFAULT}). "
                              "Each scenario+metric derives its own seed from this base, so changing it "
                              "reproducibly shifts every run's random draws together.")
    ...
    base_seed = args.seed
    metrics_to_run = OBJECTIVE_METRICS if args.metric == 'both' else [args.metric]

    if not confirm_run_plan(SCENARIOS, active_names, metrics_to_run, n_bootstrap_iterations, base_seed):
        print("Aborted -- edit ACTIVE_SCENARIOS (or pass scenario names/--iterations/--metric/--seed as arguments) and rerun.")
        sys.exit(0)
    ...
    active = [s for s in SCENARIOS if s['name'] in active_names]
    total_runs = len(active) * len(metrics_to_run)
    run_count = 0
    ...
    for scenario in active:
        for metric in metrics_to_run:
            ...
            try:
                run_scenario_metric(scenario, metric, main_byid_df, n_bootstrap_iterations, base_seed)
            except Exception:
